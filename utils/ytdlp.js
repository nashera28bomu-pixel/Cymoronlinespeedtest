...
const RAPIDAPI_KEY = process.env.RAPIDAPI_KEY    || '';   // ← add this in Render env vars
...
// ─── Main Search — 4 layers of fallback ──────────────────────────────────────
export async function searchYoutube(query, limit = 12) {

  // LAYER 1: YouTube InnerTube (their own private API, no key, no bot block)
  try {
  ...
  // LAYER 2: RapidAPI YouTube Search (reliable, 500 req/month free)
  if (RAPIDAPI_KEY) {
    try {
  ...
  // LAYER 3: Official YouTube Data API v3
  if (YT_KEY) {
    try {
...
// ─── LAYER 2: RapidAPI YouTube Search ────────────────────────────────────────
// Sign up free at https://rapidapi.com/h0p3rwe/api/youtube-search-and-download
// Add RAPIDAPI_KEY env var in Render dashboard — 500 free requests/month
